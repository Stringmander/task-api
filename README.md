...
The collection is organized as a walkthrough with no manual copy-pasting: `login`'s response script captures `accessToken`/`refreshToken`, `create-project`/`create-task`'s capture `projectId`/`taskId` — every later request in the chain references those as `{{variables}}` instead of a hardcoded id. Run the whole collection recursively (folders execute in the numbered order below) and it re-runs cleanly end to end.

- **`auth/` → `projects/` → `tasks/`** — the walkthrough itself: register, log in, create a project (plus its minimal/boundary-success variants), list it, get it, update it, create a task under it, and so on. Each request feeds the next via captured variables.
- **`tests/`** — mirrors that same `auth/`/`projects/`/`tasks/` split as subfolders, but holds the independent checks instead: validation failures (`create-project-empty-name.yml`, `create-project-name-too-long.yml`), cross-user 403s (`get-project-cross-user.yml`), missing/garbage-token 401s (`list-projects-no-token.yml`, `list-projects-garbage-token.yml`), and the project → task cascade-delete demonstration. These don't chain into each other; each is a standalone assertion using deliberately-invalid input.
- **`cleanup/`** — runs last, deleting everything the walkthrough created (`delete-task`, and one `delete-project-*` per project variant `projects/` created), since leaving rows behind would break a clean re-run.
